...
class SSHFileTransfer:
    ...
    def _validate_local_file(self, local_path: str) -> None:
        ...
        if not os.access(local_path, os.R_OK):
            raise FileTransferError(f"Cannot read file: {local_path}")
    ...
    def upload_file(
        ...
        if not self.connection_manager.is_connected():
            raise SSHConnectionError(
                f"Cannot upload file: not connected to {self.connection_manager.hostname}"
            )

        self._validate_local_file(local_path)
        ...
        try:
            ...
            raise FileTransferError(
                f"SFTP error uploading {local_path} to {remote_path}: {str(e)}"
            ) from e
        except IOError as e:
            raise FileTransferError(
                f"IO error uploading {local_path} to {remote_path}: {str(e)}"
            ) from e
        except Exception as e:
            raise FileTransferError(
                f"Unexpected error uploading {local_path} to {remote_path}: {str(e)}"
            ) from e
    ...
    def download_file(
        ...
        if not self.connection_manager.is_connected():
            raise SSHConnectionError(
                f"Cannot download file: not connected to {self.connection_manager.hostname}"
            )
        ...
        try:
            ...
            raise FileTransferError(
                f"SFTP error downloading {remote_path} to {local_path}: {str(e)}"
            ) from e
        except IOError as e:
            raise FileTransferError(
                f"IO error downloading {remote_path} to {local_path}: {str(e)}"
            ) from e
        except Exception as e:
            raise FileTransferError(
                f"Unexpected error downloading {remote_path} to {local_path}: {str(e)}"
            ) from e
    ...
    def upload_directory(
        ...
        if not os.path.isdir(local_dir):
            raise FileTransferError(f"Local path is not a directory: {local_dir}")
        ...
        try:
            ...
            for root, dirs, files in os.walk(local_dir):
                ...
                for file_name in files:
                    ...
                    self.upload_file(local_file, remote_file, show_progress)

        except FileTransferError:
            raise
        except Exception as e:
            raise FileTransferError(
                f"Error uploading directory {local_dir} to {remote_dir}: {str(e)}"
            ) from e
